...
/* --------------------------------------------- */
   ...
   fgets(dummy,80,fptr1);
     ...
     fclose(fptr1);
     printf ("starting vector is  (1 1 1 ...)\n");
     for (i = 0; i < nq; i++) {
         st[i] = 1;
         }
     printf( "1. accept the above vector\n");
     printf( "2. adjust some components\n");
     printf( "   your choice < 1 or 2 > ");
     scanf("%d", &ich);
     if (ich == 2 ) {
        printf( "adjust components");
        printf( "   < dof# = 0 exits this mode >\n");
	do {
	   printf("dof# \n");
	   scanf("%d", &n);
	   if (n > 0) {
