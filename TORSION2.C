...
/* --- equation solving --- */
   ...
   fprintf(fptr1, "%s\n", title);
   printf("%s\n", title);
   fprintf(fptr1, "node no.  stress function value\n");
   ...
   for (i = 0; i < nn; i++) {
     ...
     if (ipl == 2 ) {
       fptr2 = fopen(file3, "w");
       fprintf( fptr2,  "nodal stress function values\n");
       for (i = 0; i < nn; i++) {
	      fprintf(fptr2, " %11.4e\n", f[i]);
	      }
	   fclose(fptr2);
       ...
       printf ("run contour1 or contour2 to plot costant stress fn contours\n");
       }
     sum = 0;
     for (i = 0; i < ne; i++) {
         i1 = noc[nen*i]-1;
     ...
     printf("torque = ");
     scanf("%f", &torque);
     printf("symmetry factor (eg. if 1/4 symmetry, then = 4.0) = ");
     scanf("%f", &sfac);
     smod = pm[0];
     alpha = torque / smod / sum / sfac;
     fprintf(fptr1, "twist per unit length = %f\n", alpha);
     printf("twist per unit length = %f\n", alpha);
     fprintf(fptr1, "shearing stresses tauyz, tauxz in each element\n");
     fprintf(fptr1, "elem#   tauyz         tauxz\n");
