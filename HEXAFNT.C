...
main()
{
   ...
   int ne,nn,nq,nm,nd,nl,nen,ndn,ndim,npr,nch,nmpc,ibl;
   int mtn,mtn1,ip;
   int *noc, *nu, *mat, *mpc, *isbl, *iebl, *indx;
   float *x, *pm, *u, *tempr, *s, *f, *beta;
   float c,dj,al,tld,cnst,reaction,s1,s2,s3,pi,cal,siv1,siv2,vm;
   float se[24][24],xi[3][8],xni[3][8],d[6][6];
   float b[6][24],db[6][24],qt[24],str[6];
   if (npr < 3)
      npr = 3;
/*-------------------------------------------------------*/
   printf("\n");
...
/* ------------------------------------------------- */
   ...
   s = (float *) calloc(ibl*ibl, sizeof(float));
/* ------------------------------------------------- */
     nfron = 0;
	 ntogo = 0;
	 ndcnt = 0;
...
elstif(n,se,qt,xi,xni,d,b,db,tempr,x,al,noc)
   int n,*noc;
   float al,*x,*tempr,d[][6],qt[],xi[][8],xni[][8];
   float se[][24],b[][24],db[][24];
{
   int i,j,k,ip;
   float dj,dte,c,dsum;
...
dbmat(n,ip,x,noc,d,b,db,dj,xi,xni)
  int n,*noc,ip;
  float d[][6],b[][24],db[][24],xi[][8],xni[][8],*x,*dj;
{
 int i,j,k,kn,ir,ic;
 float aj[3][3],tj[3][3],gn[3][8],h[9][24],g[6][9];
 float dj1,dj2,dj3,c;
/* -------  db()  matrix  ------ */
